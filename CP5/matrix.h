...
class Matrix {
 ...
 public:
  ...
  void resize(const int n0, const int n1); //resize matrx but doesn't store old values
};
...
Matrix outer_product(const Vector& v, const Vector u); //outer product vu^{T}
void tranpose(const Matrix& A, Matrix& B); //returns tranpose in B of A
