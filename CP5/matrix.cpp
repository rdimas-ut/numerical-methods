...
void Matrix::resize(const int n0, const int n1) {
  if(n0<1 || n1<1)
    std::cerr << "Size too small in Matrix::resize(const int, const int)";
  if (n0!=size[0] || n1!=size[1]) {
    delete[] array;
    size[0] = n0; size[1] = n1;
    array = new double[size[0]*size[1]];
  }
...
int matVecMult(const Matrix& M, const Vector& x, Vector& y) {
  if (!(M.n(0) != x.n() || M.n(1) != x.n())) return 1;
  if (!(M.n(0) != y.n() || M.n(1) != y.n())) return 1;
...
Matrix outer_product(const Vector& v, const Vector u){
  Matrix r(v.n(), u.n());
  for (int i = 0; i < u.n(); i++){
    for (int j = 0; j < v.n(); j++){
      r(i,j) = v(i)*u(j);
...
void tranpose(const Matrix& A, Matrix& B){
  int m = A.n(0);
  int n = A.n(1);
  for (int i = 0; i < m; i++){
    for (int j = 0; j < n; j++){
